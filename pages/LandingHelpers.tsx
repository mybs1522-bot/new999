...
export const Logo = () => (
  <div className="flex flex-col items-center text-center cursor-pointer group" onClick={() => window.location.href = '/'}>
    ...
    <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-orange-700 whitespace-nowrap mt-1">Design</span>
  </div>
);
...
export const CallToActionWidget = ({ timeLeft, onClick, headline, subtext }: { timeLeft: { h: number; m: number; s: number }; onClick: () => void; headline?: string; subtext?: string }) => {
  ...
  return (
    <div className="relative py-12 md:py-20 px-6 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-orange-700/5 blur-[150px] rounded-full pointer-events-none"></div>
      <div className="max-w-2xl mx-auto relative z-10 text-center">
        ...
        <div className="mt-4 flex items-center justify-center gap-4 md:gap-8 text-[9px] md:text-[11px] font-medium uppercase tracking-[0.15em] text-zinc-500">
          ...
          <div className="flex items-center gap-1.5"><Zap size={14} className="text-orange-600" /> Instant Access</div>
          <div className="w-[1px] h-3 bg-zinc-700 hidden sm:block"></div>
          <div className="hidden sm:flex items-center gap-1.5"><Users size={14} className="text-orange-600" /> All Software Included Free</div>
        </div>
      </div>
    </div>
  );
